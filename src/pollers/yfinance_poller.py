...
from utils.setup_logger import setup_logger  # ✅ Added Logging

# ✅ Initialize Logger
logger = setup_logger(__name__)

# ✅ Initialize RateLimiter (Throttle requests)
RATE_LIMIT = int(os.getenv("YFINANCE_FILL_RATE_LIMIT", 100))
...
class YFinancePoller(BasePoller):
    ...
    def poll(self, symbols: List[str]) -> None:
        ...
        for symbol in symbols:
            try:
                rate_limiter.acquire(context=f"YFinance - {symbol}")  # ✅ Apply rate limiting
